...
**ACCEPTANCE**
- Run on the user's codebase (or any well-known Java class):
...
- [ ] `store.get_callers("$MODULE")` returns ≥3 real callers, zero hallucinations (every result has a verifiable file path + line).
- [ ] `/admin/graph` renders.
